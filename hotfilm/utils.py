...
def rdatetime(when: np.datetime64, period: np.timedelta64) -> np.datetime64:
    ...
    if mod < period_ns // 2 or mod == 0:
        when_ns -= np.timedelta64(mod, 'ns')
    else:
        when_ns += np.timedelta64(period_ns - mod, 'ns')
...
def get_repo_url() -> str:
    """
    Get the URL for the hotfilm repository, which is added to the dataset
    attributes for provenance.
    """
    return 'https://github.com/NCAR/hotfilm'


def get_version_info() -> dict:
    "Return a dictionary of version fields."
    gd = get_git_describe()
    gd = f'{gd}' if gd else ''
    rv = VERSION + (f" ({gd})" if gd else "")
    return {
        'version': VERSION,
        'repo_commit': gd,
        'repo_version': rv,
...
def add_history_to_dataset(ds: xr.Dataset, script: str,
    ...
    ds.attrs['repo_version'] = vinfo['repo_version']
    ds.attrs['repo_url'] = vinfo['repo_url']
